...
const emptyForm = () => ({ code: '', name: '', title: '', province: '', district: '', phone: '', mobilePhone: '', taxOffice: '', taxNo: '', address: '', location: '', areaM2: '', customerGroupCode: '', customerDiscount1: '', customerDiscount2: '', customerDiscount3: '', cashDiscount: '', deferredDiscount: '', active: true })
...
export function DistributorCustomersWindow() {
  ...
  const save = async () => {
    ...
    const record = { ...form, name: form.name.trim(), title: form.title.trim(), type: 'Perakende', territory: '', salesRepresentative: '', phone: form.phone.trim(), mobilePhone: form.mobilePhone.trim(), taxNo: form.taxNo.trim(), taxOffice: form.taxOffice.trim(), address: form.address.trim(), location: form.location.trim(), areaM2: form.areaM2.trim(), balance: '0,00 TL' }
  ...
  return <div className="distributor-customers-window">
    {showForm && <section className="shipment-form-panel">
      ...
      <div className="shipment-form-grid">
        ...
        <label>Müşteri Tipi<input value="Perakende" readOnly /></label>
