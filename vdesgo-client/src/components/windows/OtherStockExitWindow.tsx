...
export function OtherStockExitWindow({ onSaved }: { onSaved?: () => void }) {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${apiUrl}/inventory/other-exits/next-number`, {
        headers: authHeaders,
  ...
  useEffect(() => {
    if (!exitingWarehouseCode || !line.productCode)
      return setStockBaseQuantity(null);
    fetch(
  ...
  const saveExit = async () => {
    if (!receiptNo || !exitingWarehouseCode || lines.length === 0)
      return setMessage(
        "Fiş no, çıkan depo ve en az bir ürün satırı zorunludur.",
      );
    const response = await fetch(`${apiUrl}/inventory/other-exits`, {
      method: "POST",
